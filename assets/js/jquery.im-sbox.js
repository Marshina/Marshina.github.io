$.fn.imSbox = function() {
  ...
  if (this.length > 0) {
    ...
    if (nativeOption.eq(i).data('id')) {
          optionCustomId = nativeOption.eq(i).data('id');
        } else {
          optionCustomId = '';
        }
        ...
        customSelect.append("<div id=\"" + optionCustomId + "\" class=\"im-sbox-option " + optionCustomClass + "\" style=\"background-image: " + optionIcon + ";\">" + (nativeOption.eq(i).text()) + "</div>");
      }
      customOption = $('.im-sbox-option', parent);
      active.text(nativeOption.eq(0).text()).css('background-image', 'url("' + nativeOption.eq(0).data('icon') + '")');
      ...
      parent.on('click touchstart', '.im-sbox-option', function() {
        ...
        active.removeAttr('class').css('background-image', 'url("' + optionIcon + '")');
...
if($('.js-co-code').length) {
	$('.js-co-code').mask( $('.js-co').val() );
	mask = $('.js-co').val();
	$('body').on('change', '.js-co', function(){
		$(this).parents('.b-input-phone').find('.js-co-code').mask( $(this).val() );
		mask = $(this).val();
	});
}